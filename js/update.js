...
function update() {
	...
	//C amera pivot point smoothly follows planetToFollowPos
...
function UpdateTurnTimer() {
	...
	if (turnTime - Math.floor(turnTimer) != turnTimeShown && turnTimeShown >= 0) {
		...
		// #HACK# Webkit browsers update innerhtml only once
	...
	$.each(objects.projectiles, function(idx, obj) {
		...
		if (obj.splineTime >= 1) {
			...
			$.each(objects.units, function(unitIdx, unit) {
				...
				if (temp.length() < 100) { // explosion size
					scene.remove(unit);
...
function MoveUnit(unit, position, clickedObject) {
	// minor fix: collision boxes between units (just simple distance calc over objects.units...)
	// (prevent moving inside other unit)
